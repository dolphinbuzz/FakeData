...
A auditoria de seletores informa problemas diretamente com um ícone vermelho ao lado do campo, cujo tooltip explica a correção. O botão **Copiar JSON**, na aba **Automático**, copia um objeto `chave: valor` com os locators nomeados e seus seletores. Seletores priorizam primeiro o `id` existente e único do elemento, seguido por `data-cy`, `data-test`, `data-testid`, atributos ARIA/semânticos, `value` (com igualdade exata e depois parcial), `ng-model`, `name` e `type`; combinações de `ng-model`, `value`, `name` e `type` resolvem colisões antes do fallback. Classes e estilos não são usados. Um fallback estrutural posicional pode ser usado apenas para manter a operação de campos sem atributos estáveis e fica sempre marcado como frágil.
